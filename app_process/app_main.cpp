...
public:
    ...
    virtual void onVmCreated(JNIEnv* env)
    {
        if (mClassName.isEmpty()) {
            if(elpolloJNI::useFramework()) {
                elpolloJNI::ElPolloRuntime::onVmCreated(env);
            }
...
{
    // We need to modify the selinux policy for some features of
    // elpollo to work.
    elpolloJNI::modifySepolicy();
    ...
    if (zygote) {
        if(elpolloJNI::useFramework()) {
            elpolloJNI::addClassPathEntry();

            runtime.start(elpolloJNI::ZYGOTE_INIT, args);
        } else {
